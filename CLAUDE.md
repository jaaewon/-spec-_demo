...
- ~~**경제지표 조회** (`data/indicators.csv`) → 프롬프트에 주입~~ — **미구현 / 미확정 (§16-15).**
  이 CSV 를 읽는 코드가 없다. 아래 '제외' 의 "경제지표의 소비 — 프롬프트 주입" 과
  정면으로 어긋나 있었다. 현행 코드는 '제외' 쪽(프롬프트에 안 넣음)과 일치한다.
- 로컬 LLM 자체 판단 → Spec 생성 (구조화 출력 강제)
- Spec 스키마 검증 (Pydantic) + 유니버스/레버리지 검증 (`validators.py`)
- ~~**시스템 하드캡 검수** (`data/hardcaps.json`) → 위반 시 거부~~ — **미구현 / 미확정 (§16-14).**
  이 JSON 을 읽는 코드가 없다. 실제로 도는 하드캡은 아래 `hardcap_profile` 기반
  **클램프**이고, 성향별 상한(conservative 8 / neutral 15 / aggressive 30)은 어디에도
  적용되지 않는다. 두 설계 중 무엇으로 통일할지는 팀 결정 (§8.3, §16-14).
...
파이프라인 순서 (**현행 코드**): `설문 → 유니버스 주입 → LLM Spec 생성 → 스키마·유니버스 검증`
` → 하드캡 클램프/반려 → snapshot_date 로 지표 as-of 조회(기록만) → 저장`
지표 조회는 프롬프트 **앞**이 아니라 Spec 생성 **뒤**에 있고, 프롬프트에 들어가지 않는다 (§17.2).
...
- 하드캡 **스텁 3종** — `hardcap_profile` 에 값은 있으나 검증 로직은 "판정 불가"를 반환한다.
  ...
  | 항목 | 미적용 사유 | 재검토 시점 |
  |---|---|---|
  | MDD 상한 | ~~데모에 백테스트 계층이 없어 산출 불가~~ → **사유 소멸 (§16-16).** `app/backtest.py` 가 `max_drawdown_pct` 를 실제로 산출한다. 다만 하드캡 값이 잠정치이고 백테스트의 범위 편입도 미확인이라 **판정은 여전히 스텁으로 둔다** | 범위·캡 값 확정 후 |
...
- 실전 거래 / 승인 게이트
- ~~백테스팅 / 리포트 생성~~ — **구현되어 있으나 범위 승인 미확인 (§16-16).**
  `app/backtest.py`(vectorbt) · `app/tickers.py`(pykrx) · `POST /backtest/{spec_id}` ·
  프론트 차트/리포트가 동작한다. 범위 편입이 팀 승인을 거쳤는지 확인되지 않아
  기능을 들어내지도, '포함' 으로 옮기지도 않고 표시만 해 둔다.
...
[FastAPI]                    ┌── 0) 결정적 사전 스캔 (§19) — LLM 아님
  ...
  2) [미구현] indicators.csv 조회 → 프롬프트에 주입 — 코드에 없다 (§16-15)
...
[FastAPI] 저장된 Spec → pykrx 시세 → vectorbt 백테스트 → 리포트·plotly figure
          (app/backtest.py, app/tickers.py — 범위 승인 미확인, §16-16)
```
...
> 두 소스를 어떻게 정리할지는 미결정 (§16-15). 파일은 지우지 않는다.
...
### 8.3 정적 하드캡 규칙 (`data/hardcaps.json`) — **미구현 / 미확정**

> ⚠️ **이 절은 설계 메모다. `data/hardcaps.json` 을 읽는 코드는 한 군데도 없다.**
> 실제로 도는 4계층은 §8.2 / §18 의 `hardcap_profile` 기반 **클램프**이고,
> 아래 표의 성향별 상한·종목 수·신호 수 제한은 **어디에서도 검사되지 않는다.**
> 두 설계 중 무엇으로 통일할지는 팀 결정이다 (§16-14). 파일은 지우지 않는다.
...
│   └── backtest.py           # Spec → vectorbt 백테스트 → 리포트·차트 (범위 승인 미확인, §16-16)
├── db/
...
│   ├── intent_lexicon.json        # 사전 스캔 어휘 + 항목별 근거 (§19). 유니버스와 동기화 필요 (§16-18)
│   ├── indicators.csv             # 종목별 기술지표 더미 — **읽는 코드 없음** (§7.1, §16-15)
│   └── hardcaps.json              # 정적 하드캡 규칙 — **읽는 코드 없음** (§8.3, §16-14)
└── static/
    └── index.html
```
...
- ~~하드캡: "안정형 + 감내 손실 10%" → 상한(8%) 위반으로 거부 (§8.3)~~
  → **현행 코드에서 재현 불가.** 실측 200 · `clamps: []`. 성향별 상한은 `hardcaps.json`
  쪽 설계인데 읽는 코드가 없고, 도는 캡은 성향 무관 단일 상한 20% 다 (§16-14).
- ~~하드캡: "안정형 + 주 1회 리밸런싱" → 과도하게 잦아 거부~~
  → **재현 불가.** 실측 200 · 조정 없음. 같은 이유이며, `min_rebalance_days=7` 은
  현행 enum 최소 단위와 같아 어떤 응답도 위반할 수 없다 (§18.1).
- ~~지표 반영: "2차전지 + 모멘텀" → rationale 에 지표 판단이 드러나는지~~
  → **재현 불가.** 지표는 프롬프트에 들어가지 않으므로(§17.2) LLM 출력에 영향을 줄 수 없다.
  `indicators.csv` 는 읽는 코드조차 없다 (§7.1, §16-15).
...
| # | 입력 | 기대 결과 |
|---|---|---|
| 1 | "반도체 쪽에 관심이 많은데 공격적으로 가고 싶어요. 손실은 10%까지는 감수할 수 있습니다. **배당주도 조금 섞어**주면 좋겠어요." | **200.** 반도체 ETF + 배당 ETF 가 **한 Spec 에** 담긴다. `sector` 단수 enum(경로 1)으로는 표현 불가능한 의도다. 실측: `['TIGER Fn반도체TOP10', 'KODEX 고배당주', 'KODEX 코리아배당성장']` (2026-08-29 / qwen3:8b). **어느 종목이 뽑히는지는 LLM 출력이라 고정이 아니다** — 이 시나리오가 주장하는 것은 두 테마가 한 Spec 에 공존한다는 사실이다 |
...
# → ['KODEX 고배당주', 'KODEX 코리아배당성장', 'RISE 고배당'] ['코인']
#   (실측 2026-08-29 / qwen3:8b / 유니버스 20종. **종목 구성은 LLM 출력이라 재현이
#    보장되지 않는다** — temperature=0 이어도 모델·유니버스가 바뀌면 달라진다.
#    이 시연이 고정으로 주장하는 것은 종목명이 아니라 ① 200 으로 통과했고
#    ② notices 에 '코인'이 남았다는 두 가지다.)
...
13. **기록 테이블(`requests` / `specs`)의 저장 shape 변경 시 마이그레이션 정책**
   ...
   **이번 판단을 선례로 굳히지 않는다.**
14. **하드캡 계층 이중화** — 같은 계층에 설계가 두 벌 있다. 어느 쪽으로 통일할지 결정 필요.
   | | `data/hardcaps.json` | `hardcap_profile` 테이블 |
   |---|---|---|
   | 읽는 코드 | **없음** | `db.load_active_hardcap_profile()` (요청마다) |
   | 값의 위치 | 정적 파일 | DB, 버전 INSERT 로 무중단 교체 (§18.3) |
   | 캡 기준 | **성향별** (conservative 8 / neutral 15 / aggressive 30) | 성향 무관 단일 상한 20 |
   | 위반 처리 | 전부 **거부 400** | **클램프 200** + `clamps`, 구조적 위반만 400 |
   | 검사 항목 | max_loss / etfs 수 / signals 수 / 성향별 리밸런싱 주기 | max_loss / 리밸런싱 최소 간격 + 스텁 3종 |
   기획서 4.1 정합성: 4계층 자체는 양쪽 다 구현하지만, "수치는 클램프" 원칙(§18.2, §8.2)은
   `hardcap_profile` 쪽에만 있다. 반대로 **종목 수·신호 수 상한과 성향별 차등은 `hardcaps.json`
   에만 있고 현재 어디에서도 검사되지 않는다** — 통일 방향과 무관하게 이 두 항목은 유실 상태다.
   **파일은 지우지 않는다.**
15. **지표 소스 이중화** — 성격이 다른 두 소스가 공존한다.
   | | `data/indicators.csv` | `data/economic_indicators.json` → DB |
   |---|---|---|
   | 내용 | 종목별 기술지표 (종가·모멘텀·RSI·이평교차) | 거시지표 5종 (기준금리·환율·국고채·CPI·FFR) |
   | 읽는 코드 | **없음** | `app/indicators.py` (as-of 조회, §17) |
   | 쓰임 | 프롬프트 주입 전제 (미구현) | 조회·기록만. 프롬프트에 안 넣음 |
   추가로 `app/backtest.py` 가 같은 이름의 기술지표 3종을 **pykrx 실시세에서 직접 계산**한다.
   즉 기술지표의 실질 소비자는 CSV 가 아니라 백테스트다. §16-5 에서 "지표 소비는 범위 밖"으로
   확정한 것과 §2 포함 목록의 "프롬프트 주입" 이 충돌하므로 정리 필요.
16. **백테스트의 범위 편입 여부** — `app/backtest.py`·`app/tickers.py`·`POST /backtest/{spec_id}`
   가 동작하지만 §2 는 여전히 "백테스팅 제외"다. 범위 편입이 승인되면 따라오는 결정:
   - **MDD 하드캡 활성화** — 스텁 사유("백테스트 계층이 없어 산출 불가")가 이미 소멸했다.
     활성화하려면 `mdd_pct_cap = 30` 의 절대값 근거와, 백테스트 실행이 `/compile` 경로에
     들어가는지(지금은 별도 요청) 부터 정해야 한다.
   - 백테스트 결과를 `specs` 에 박제할지 (현재는 저장하지 않고 응답으로만 나간다).
   - 백테스트가 정한 해석(신호 OR 결합·리밸런싱을 매매 가능일 제한으로·손절선·균등 배분)의 승인.
17. **유니버스 종목코드 검증** — `data/etf_universe.csv` 의 code 가 그 name 의 실제
   종목코드인지 레포 안에서 검증할 수 없다 (pykrx 이름 조회 API 가 빈 값). 20종 전부 시세는
   조회되지만 이는 "상장 종목"이라는 뜻일 뿐이다. **알려진 불일치: `211900`** — 구 티커 매핑은
   `TIGER 배당성장`, 현행 csv 는 `KODEX 코리아배당성장`. 지금은 csv 를 단일 출처로 따르고 있다.
   KRX 원본 대조가 필요하다.
18. **유니버스 변경 시 `intent_lexicon.json` 동기화** — 현재는 **수동이며 자동 검증
   수단이 없다.** 사전 스캔의 섹터 positive 는 유니버스 `theme` 에서 파생되므로 theme
   집합이 그대로면 로드 시 정합성 검사(`_check_keys`)를 통과한다. 그런데 **theme 이 같아도
   종목이 바뀌면 어휘는 낡는다** — 검사가 보는 것은 theme 키뿐이라 이 낡음을 잡지 못한다.
   실제로 13종 → 20종 확장에서 두 건이 나왔고(§19.3 의 D-1·D-2), 둘 다 셀프체크를 통과한
   채로 발견됐다:
   - `미국채10년선물`(theme `채권`) 추가 → `미국주식` 동의어 `"미국"` 이 `"미국채"` 에 걸려
     **오탐**. `"미국채"` 를 `채권` 동의어로 추가해 **긴 표현 우선 규칙으로** 해소했다.
   - `KODEX 코스피100`(theme `대형주`) 추가 → `대형주` 동의어에 `코스피200` 계열만 있어
     **미탐**(명시한 슬롯이 `inferred` 로 찍힘). `"코스피100"` 을 추가해 해소했다.
   검토 대상: 유니버스의 **종목명**까지 훑어 렉시콘이 못 잡는 표현을 찾는 셀프체크를
   만들지, 아니면 동기화를 릴리스 체크리스트 항목으로 둘지.
19. **`rebalance` 슬롯이 `clamps` 를 보지 않는다 (§19.3.1)** — `_check_enum_slot()` 은
   `max_loss` 와 달리 하드캡 조정 내역을 인자로 받지 않는다. 지금은 `min_rebalance_days=7`
   이 `RebalanceFreq` 최소 단위 `weekly` 와 같아 클램프가 발동하지 않으므로 **잠복 상태**다.
   §18.1 이 "캡을 8 이상으로 올리면 즉시 발동"이라고 적어둔 그 지점에서, 발동하는 순간
   "주 1회"라고 말한 사용자에게 **허위 `conflict`** 가 나간다 (`implies: weekly` vs
   클램프된 `spec_value: monthly`). 고치려면 `_check_slot` → `_check_enum_slot` 의 시그니처를
   바꿔야 해서 이번 머지 범위 밖으로 뒀다. **캡 값 변경은 팀 결정 사항이므로 그 결정과
   함께 처리해야 한다** — 캡만 올리고 이걸 놔두면 정상 응답이 매번 conflict 로 표시된다.
...
| **MDD 상한** `mdd_pct_cap` | **30%** | `1회 손실 한도(20) < MDD 상한(30)` 관계 유지, 그것 하나가 유일한 근거. 절대값은 **P3 에서 유니버스 실측 MDD 확인 후 확정** | **잠정 (절대값 근거 없음)** | ⛔ 스텁 — 단 **사유가 바뀌었다.** ~~백테스트 계층 부재~~ → `app/backtest.py` 가 MDD 를 산출한다. 지금 스텁인 이유는 **캡 절대값에 근거가 없고 백테스트의 범위 편입도 미확인**이기 때문 (§16-16) |
| **최소 리밸런싱 간격** `min_rebalance_days` | **7일** | 현행 `RebalanceFreq` 최소 단위 `weekly`(7일)와 **일부러 같게** 잡았다. 더 크게 잡으면 설문의 '주 1회' 선택지가 **항상** 클램프돼 정상 응답이 매번 조정된다 | **잠정** | ⛔ 미발동 — 로직은 살아 있고, enum 에 더 짧은 주기가 추가되거나 캡을 8↑ 로 올리면 즉시 발동. **⚠️ 아래 경고 참고** |

> ⚠️ **캡을 올리면 `rebalance` 슬롯에서 허위 `conflict` 가 발생한다** (§16-19).
> `intent._check_enum_slot()` 이 `clamps` 를 보지 않기 때문이다 — `max_loss` 는
> 하드캡 조정 **전** 값과 대조하도록 되어 있지만(§19.3.1) `rebalance` 는 그 처리가 없다.
> 그래서 "주 1회"라고 말한 사용자에게 `implies: weekly` vs 클램프된
> `spec_value: monthly` 로 어긋남이 표시된다. 지금은 캡이 `weekly` 와 같아 클램프가
> 발동하지 않으므로 잠복 상태다. **캡 변경 전에 함께 고쳐야 한다** — 캡 값 변경은
> 팀 결정 사항이므로, 그 결정 시점에 이 결함이 함께 보이도록 여기에 적어 둔다.
...
> **MDD 스텁 사유 갱신 (2026-08-29).** 이 표와 §2 는 원래 "데모에 백테스트 계층이 없어
> MDD 를 산출할 수 없다"를 사유로 들었다. 통합으로 `app/backtest.py` 가 들어오면서 그 사유는
> 소멸했다 — `BacktestResult.max_drawdown_pct` 가 실제 값이다. **그럼에도 판정을 켜지 않는 이유**는
> (a) `mdd_pct_cap = 30` 의 절대값에 근거가 없고, (b) 백테스트가 `/compile` 과 별도 요청이라
> Spec 생성 시점에는 여전히 MDD 를 모르며, (c) 백테스트의 범위 편입 자체가 미승인이기 때문이다.
> 셋 다 팀 결정 사항이라 여기서는 사유만 갱신하고 구현하지 않는다 (§16-16).
...
> 마지막 항목의 한계 — 이유가 둘 겹쳐 있다: (a) `data/etf_universe.csv` 에 레버리지/인버스
...
| 어휘 | 출처 | 왜 |
|---|---|---|
| 섹터 positive | **`data/etf_universe.csv` 의 `theme` 에서 파생** | 유니버스를 늘렸을 때 두 곳이 어긋나지 않게. 렉시콘은 동의어만 덧붙이고, 그 키가 실재하는 theme 인지 로드 시점에 검증한다 |
...
> ⚠️ **파생시켰다고 동기화가 자동이 되는 것은 아니다** (§16-18). 로드 시 정합성 검사가
> 보는 것은 theme **키**뿐이라, theme 집합이 그대로인 채 **종목만 바뀌면 어휘가 낡아도
> 통과한다.** 실제로 유니버스 13종 → 20종 확장에서 두 건이 셀프체크를 전부 통과한 채
> 발견됐고, 둘 다 사용자에게 보이는 오류였다:
>
> | 종목 추가 | 증상 | 원인 |
> |---|---|---|
> | `미국채10년선물` (theme `채권`) | 정상 입력 "미국채 10년물"의 sector 가 `미국주식` 으로 찍혀 최종 Spec 과 **허위 conflict** | `미국주식` 동의어 `"미국"` 이 `"미국채"` 에 부분문자열로 걸린다 |
> | `KODEX 코스피100` (theme `대형주`) | 사용자가 **명시한** 슬롯이 `source: inferred` 로 찍힘 — `slots` 의 목적 자체를 훼손 | `대형주` 동의어에 `코스피200` 계열만 있었다 |
>
> **이것도 §19.3 계열의 한계다** — 어휘 매칭이 문자열만 보고 뜻을 읽지 못한다는 하나의
> 원인에서, 요구/언급 구별 불가(§19.3)와 `conflict` 원인 구별 불가(§19.3.1)와 함께 나온다.
> 다만 실패 방향이 또 다르다: 요구/언급은 잘못 판정하면 **400 으로 요청이 죽고**,
> `conflict` 원인은 판정을 **유보**하는데, 이쪽은 **조용히 틀린 값을 확신 있게 표시한다.**
> 셋 중 가장 눈에 안 띄는 실패라 셀프체크를 통과한 채 남아 있었다.
>
> 위 두 건은 렉시콘에 `"미국채"`(→채권) · `"코스피100"`(→대형주)을 추가해 해소했다.
> `"미국채"` 는 **긴 표현 우선 규칙**에 의존한다 — `"미국채"` 가 `"미국"` 보다 길어 먼저
> 매치된다. 이 규칙은 원래 `_find()` 안, 즉 **한 테마의 terms 안에서만** 돌았고 테마
> 사이 동점은 위치로만 갈라 승자가 `sorted(theme)` 순서로 정해졌다(그래서 `미국주식` 이
> `채권` 을 이겼다). 어휘 추가와 함께 `_scan_slots` 의 정렬 키를 `(위치, -길이)` 로 고쳐
> 같은 규칙을 테마 사이에도 적용했다. **유니버스에 미국 관련 종목이 더 늘면 같은 유형의
> 오탐이 재발할 수 있다.**
