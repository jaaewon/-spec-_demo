...
- ~~**경제지표 조회** (`data/indicators.csv`) → 프롬프트에 주입~~ — **미구현 / 미확정 (§16-9).**
...
- 하드캡 **스텁 3종** — `hardcap_profile` 에 값은 있으나 검증 로직은 "판정 불가"를 반환한다.
  ...
  | 항목 | 미적용 사유 | 재검토 시점 |
  |---|---|---|
  | MDD 상한 | ~~데모에 백테스트 계층이 없어 산출 불가~~ → **사유 소멸 (§16-10).** `app/backtest.py` 가 `max_drawdown_pct` 를 실제로 산출한다. 다만 하드캡 값이 잠정치이고 백테스트의 범위 편입도 미확인이라 **판정은 여전히 스텁으로 둔다** | 범위·캡 값 확정 후 |
...
- 실전 거래 / 승인 게이트
- ~~백테스팅 / 리포트 생성~~ — **구현되어 있으나 범위 승인 미확인 (§16-10).**
...
> 두 설계 중 무엇으로 통일할지는 팀 결정이다 (§16-8). 파일은 지우지 않는다.
...
- ~~하드캡: "안정형 + 감내 손실 10%" → 상한(8%) 위반으로 거부 (§8.3)~~
  ...
  쪽 설계인데 읽는 코드가 없고, 도는 캡은 성향 무관 단일 상한 20% 다 (§16-8).
...
- ~~지표 반영: "2차전지 + 모멘텀" → rationale 에 지표 판단이 드러나는지~~
  ...
  `indicators.csv` 는 읽는 코드조차 없다 (§7.1, §16-9).
...
8. **하드캡 계층 이중화** — 같은 계층에 설계가 두 벌 있다. 어느 쪽으로 통일할지 결정 필요.
...
10. **백테스트의 범위 편입 여부** — `app/backtest.py`·`app/tickers.py`·`POST /backtest/{spec_id}`
...
11. **유니버스 종목코드 검증** — `data/etf_universe.csv` 의 code 가 그 name 의 실제
...
| **MDD 상한** `mdd_pct_cap` | **30%** | `1회 손실 한도(20) < MDD 상한(30)` 관계 유지, 그것 하나가 유일한 근거. 절대값은 **P3 에서 유니버스 실측 MDD 확인 후 확정** | **잠정 (절대값 근거 없음)** | ⛔ 스텁 — 단 **사유가 바뀌었다.** ~~백테스트 계층 부재~~ → `app/backtest.py` 가 MDD 를 산출한다. 지금 스텁인 이유는 **캡 절대값에 근거가 없고 백테스트의 범위 편입도 미확인**이기 때문 (§16-10) |
...
> 셋 다 팀 결정 사항이라 여기서는 사유만 갱신하고 구현하지 않는다 (§16-10).
