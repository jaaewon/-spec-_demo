...
- `data/etf_universe.csv`: 비레버리지 KRX ETF 스타터 (§7, 현재 20종, `name,code,theme`).
  종목코드는 KRX에서 검증 후 확정 — **아직 미검증** (CLAUDE.md §16-17)
...
CLAUDE.md §16-16 의 미결 항목이다. 아래는 **이미 구현된 것을 기록한 것**이다.
...
- 유니버스를 바꾸면 티커 매핑이 자동으로 따라오지만, `csv` 의 `code` 가 그 `name` 의
  실제 종목코드인지는 레포 안에서 검증할 수 없다 (§16-17)
...
미확정이라 스텁 유지 — CLAUDE.md §16-16) · 최소 리밸런싱 간격(현행 enum 최소 단위가 캡과 동일해 미발동) ·
...
- `data/intent_lexicon.json`: 사전 스캔 어휘 + **항목별 근거(reason 필수)**.
  섹터 positive 는 여기 없다(etf_universe.csv 의 theme 에서 파생, 키 정합성 로드 시 검증).
  단 그 검사는 theme **키**만 본다 — 유니버스 종목이 바뀌면 어휘는 수동으로 맞춰야 한다
  (13종 → 20종 확장에서 오탐·미탐 1건씩 발생. CLAUDE.md §16-18)
...
- 백테스팅은 M6 으로 **구현은 들어왔으나 범위 편입 승인은 미확인** (CLAUDE.md §16-16).
